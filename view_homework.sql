...
CREATE VIEW student_subject_score
AS
SELECT stu.name,sub.subject,s.score
FROM score s
INNER JOIN student stu ON stu.id = s.student_id
...
CREATE OR REPLACE VIEW student_subject_score
AS
SELECT stu.name,sub.subject,sub.teacher,s.score
FROM score s
INNER JOIN student stu ON stu.id = s.student_id
INNER JOIN `subject` sub ON sub.id = s.subject_id;
